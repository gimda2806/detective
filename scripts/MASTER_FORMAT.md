...
규모입니다. 짧고 성긴 마스터는 그 자체로 실패작으로 취급합니다 —
CASE901 예시가 실제로 이 수준입니다:

| 섹션 | 기준 |
|---|---|
| `LOCATIONS` | 5개로 고정 (6개 이상으로 늘리지 않는다) |
| `CHARACTERS` (피해자 제외 핵심 인물) | 5명으로 고정 (6명 이상으로 늘리지 않는다) |
| `EVIDENCE` | 6개 이상 |
| `ACTUAL_TIMELINE` | 10개 이상의 원자적 항목 |
| `CONTRADICTION_STAGES` | 3개 이상 |
| `RED_HERRINGS` | 2~3개 |

`LOCATIONS`와 `CHARACTERS`는 개수를 늘려서 규모를 키우지 않습니다 —
장소·인물이 늘어날수록 GM이 매 턴 참고해야 할 문맥이 커지고 관리도
어려워지므로, 여기서는 개수를 5로 고정하고 대신 각 블록의 내용(단서,
진술, hidden_until)을 더 촘촘하게 채워서 밀도를 높입니다. 나머지
섹션(증거·타임라인·모순 단계·오답)은 여전히 최소 개수만 정해져 있고
그 이상은 얼마든지 늘려도 됩니다.
...
`[OPENING_SCENE]`과 같은 방식으로 씁니다 — 번호를 매긴 요약 목록이
아니라, 사건의 전말이 자연스럽게 드러나는 **하나의 엔딩 장면**을
현재진행형 산문으로 서술합니다. 대사와 행동을 통해 진범이 왜 그런
선택을 했는지, 어떻게 덜미가 잡혔는지 보여주고, `[FULL_TRUTH]`의 핵심
사실(책임자·동기·수법·결정적 연결)을 그 장면 안에 빠짐없이 녹여내야
합니다.

```
현장이 정리되는 사이, 차유라는 매니저용 대기 의자에 앉아 두 손을 무릎 위에 모은 채 아무 말도 하지 않는다.
탐정이 서비스함 태그 기록과 정산 메모를 나란히 내려놓자 그녀의 시선이 흔들린다. ...
"이사회에 알리겠다고... 그 한마디 때문에요." 그녀가 낮게 말한다. "하린이 지금 겨우 무대로 돌아왔는데, 제가 손댄 돈 때문에 전부 무너지는 걸 볼 수 없었어요."
21시 8분, 대기실 문턱에서 그녀는 하린의 안정제 한 알을 가져와 백정우의 심장약과 바꿔 넣었다. ...
```

`app/game.ts`는 이 섹션의 내용을 GM이 케이스 종료(최종 지목 성공)
turn에서 `master.raw_text` 전체와 함께 읽고, 그걸 참고해서 엔딩
설명을 즉석에서 작성하는 데 씁니다 — 별도 필드로 파싱해서 그대로
출력하는 게 아니라, GM이 읽고 재구성하는 참고 산문입니다. 그러니
장면으로 쓰여 있어야 GM도 자연스러운 엔딩을 만들 수 있습니다.
