...
function buildGenerationInstructions(caseId: string) {
  return [
    ...
    '- [OPENING_SCENE]을 "탐정은 [의뢰인]의 다급한 연락/신고를 받고 왔다"류의 상투적 호출 문구로 시작하지 마라. 이 표현이 사건마다 반복되면 오프닝의 첫인상이 전부 똑같아져 재미를 해친다(기계적으로 걸러진다). 대신 사건 현장의 소리·대화·분위기 대비, 이미 벌어지고 있는 상황을 목격하는 방식 등 사건마다 다른 방식으로 열어라 — 연락을 받고 온 경위 자체가 필요하면 이후 문장에서 짧게만 처리하라.',
